...
class MarketScanner:
    ...
    def analyze_and_alert(self, symbol):
        ...
        try:
            ...
            limit = max(SQUEEZE_LOOKBACK_PERIOD, BREAKOUT_LOOKBACK_PERIOD, TREND_FILTER_EMA_PERIOD) + 5
            ...
            if ALERT_ON_BREAKOUT_VOLUME: self.check_volume_breakout(symbol, df)
            if ALERT_ON_TREND_FILTERED_CROSS: self.check_trend_filtered_cross(symbol, df)
            if ALERT_ON_BBAND_SQUEEZE: self.check_bband_squeeze(symbol, df)
    ...
    def check_volume_breakout(self, symbol, df):
        ...
        if symbol in self.alerted_symbols.get("volume_breakout", []): return
        last = df.iloc[-2]
        ...
        if volume_avg and last['volume'] > volume_avg * VOLUME_THRESHOLD:
            ...
            breakout_type = "Rompimento de Alta 📈" if last['close'] > highest_high else "Rompimento de Baixa 📉" if last['close'] < lowest_low else None
            if breakout_type:
                ...
                self.alerted_symbols.setdefault("volume_breakout", set()).add(symbol)
    ...
    def check_trend_filtered_cross(self, symbol, df):
        ...
        if symbol in self.alerted_symbols.get("ema_cross", []): return
        current, previous = df.iloc[-2], df.iloc[-3]
        ema_fast, ema_slow, ema_trend = current[f'EMA_{EMA_FAST_PERIOD}'], current[f'EMA_{EMA_SLOW_PERIOD}'], current[f'EMA_{TREND_FILTER_EMA_PERIOD}']
        distance_percent = (abs(ema_fast - ema_slow) / ema_slow) * 100
        are_close = distance_percent < PROXIMITY_THRESHOLD_PERCENT
        are_converging = abs(ema_fast - ema_slow) < abs(previous[f'EMA_{EMA_FAST_PERIOD}'] - previous[f'EMA_{EMA_SLOW_PERIOD}'])
    ...
    def check_bband_squeeze(self, symbol, df):
        ...
        if symbol in self.alerted_symbols.get("bband_squeeze", []): return
        df['bband_width'] = (df[f'BBU_{BBAND_LENGTH}_{BBAND_STD_DEV}'] - df[f'BBL_{BBAND_LENGTH}_{BBAND_STD_DEV}']) / df[f'BBM_{BBAND_LENGTH}_{BBAND_STD_DEV}']
        last_width = df['bband_width'].iloc[-2]
        min_width_in_period = df['bband_width'].iloc[-(SQUEEZE_LOOKBACK_PERIOD+2):-2].min()
